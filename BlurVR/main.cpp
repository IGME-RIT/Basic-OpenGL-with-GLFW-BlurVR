...
int main(int argc, char **argv)
{
	...
	while (!glfwWindowShouldClose(window))
	{
        ...
        if (benchmarkThisFrame)
        {
            ...
            glGetQueryObjectui64v(queryID[3], GL_QUERY_RESULT, &endEye2);
            glGetQueryObjectui64v(queryID[4], GL_QUERY_RESULT, &endBlur);

            system("cls");
            printf("First Eye: %f ms\n", (endEye1 - startEye1) / 1000000.0);
            printf("Second Eye: %f ms\n", (endEye2 - endEye1) / 1000000.0);
            if (!useBlur)
            {
                printf("Full Frame: %f ms\n", (endEye2 - startTime) / 1000000.0);
            }
            else
            {
                printf("Blur eyes: %f ms\n", (endBlur - endEye2) / 1000000.0);
